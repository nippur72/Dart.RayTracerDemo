...
var Sphere = (function (_super) {
    ...
    Sphere.prototype.Intersect = function (ray) {
        var lightFromOrigin = Vector3f.operator_sub(this.position, ray.origin); // dir from origin to us
        var v = lightFromOrigin.Dot(ray.direction); // cos of angle between dirs from origin to us and from origin to where the ray's pointing
...
var RayTracer = (function () {
    ...
    RayTracer.RenderRow = function (canvas, dotPeriod, y) {
        ...
        setTimeout(function () { return _this.RenderRow(canvas, dotPeriod, y + 1); }, 0);
    };
    ...
    RayTracer.Trace = function (ray, traceDepth) {
        ...
        var viewerDir = Vector3f.operator_sub_unary(ray.direction); // Direction back to the viewer (simply negative of ray dir)
    ...
    RayTracer.PI = 3.1415926536; // maths constants
    RayTracer.PI_X_2 = 6.2831853072;
    RayTracer.PI_OVER_2 = 1.5707963268;
    RayTracer.CANVAS_WIDTH = 640; // output image dimensions
    RayTracer.CANVAS_HEIGHT = 480;
    RayTracer.TINY = 0.0001; // a very short distance in world space coords
    RayTracer.MAX_DEPTH = 3; // max recursion for reflections
    RayTracer.MATERIAL_DIFFUSE_COEFFICIENT = 0.5; // material diffuse brightness
    RayTracer.MATERIAL_REFLECTION_COEFFICIENT = 0.5; // material reflection brightness
    RayTracer.MATERIAL_SPECULAR_COEFFICIENT = 2.0; // material specular highlight brightness
    RayTracer.MATERIAL_SPECULAR_POWER = 50.0; // material shininess (higher values=smaller highlights)
    RayTracer.BG_COLOR = Color.BlueViolet; // scene bg colour
    RayTracer.eyePos = new Vector3f(0, 0, -5.0); // eye pos in world space coords
    RayTracer.screenTopLeftPos = new Vector3f(-6.0, 4.0, 0); // top-left corner of screen in world coords
    RayTracer.screenBottomRightPos = new Vector3f(6.0, -4.0, 0); // bottom-right corner of screen in world coords
